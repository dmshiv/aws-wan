...
# 🚀 Robust execution engine with ERROR OUTPUT
function run_terraform() {
  ...
  local start=$(date +%s)
  if terraform init -input=false -no-color >/dev/null 2>/tmp/tf_error; then
    local duration=$(( $(date +%s) - start ))
    printf " ${GREEN}OK${NC} (%02dm%02ds)]" $((duration/60)) $((duration%60))
  else
    local duration=$(( $(date +%s) - start ))
    printf " ${RED}FAIL${NC} (%02dm%02ds)]\n" $((duration/60)) $((duration%60))
    echo -e "${RED}❌ INIT ERROR in $display_path:${NC}"
    cat /tmp/tf_error
    popd >/dev/null
    return 1
  fi
  ...
  if [[ "$action" == "destroy" ]]; then
    # 2a. DESTROY FLOW (init → destroy, no plan needed)
    echo -ne " [check..."
    start=$(date +%s)
    ...
    echo -ne " [destroy..."
    start=$(date +%s)
    if terraform destroy -auto-approve -input=false -no-color >/dev/null 2>/tmp/tf_error; then
      duration=$(( $(date +%s) - start ))
      printf " ${GREEN}DONE${NC} (%02dm%02ds)]\n" $((duration/60)) $((duration%60))
    else
      duration=$(( $(date +%s) - start ))
      printf " ${RED}FAIL${NC} (%02dm%02ds)]\n" $((duration/60)) $((duration%60))
      echo -e "${RED}❌ DESTROY ERROR in $display_path:${NC}"
      cat /tmp/tf_error
      ...
      if terraform destroy -auto-approve -input=false -no-color >/dev/null 2>/tmp/tf_error; then
        ...
        duration=$(( $(date +%s) - start ))
        printf " ${RED}FAIL${NC} (%02dm%02ds)]\n" $((duration/60)) $((duration%60))
        echo -e "${RED}❌ DESTROY RETRY ERROR in $display_path:${NC}"
        cat /tmp/tf_error
        popd >/dev/null
        return 1
      fi
    ...
    # 2b. APPLY FLOW (init → plan → apply)
    echo -ne " [plan..."
    start=$(date +%s)
    if terraform plan -input=false -no-color -out=tfplan >/dev/null 2>/tmp/tf_error; then
      duration=$(( $(date +%s) - start ))
      printf " ${GREEN}OK${NC} (%02dm%02ds)]" $((duration/60)) $((duration%60))
    else
      duration=$(( $(date +%s) - start ))
      printf " ${RED}FAIL${NC} (%02dm%02ds)]\n" $((duration/60)) $((duration%60))
      echo -e "${RED}❌ PLAN ERROR in $display_path:${NC}"
      cat /tmp/tf_error
      popd >/dev/null
      return 1
    fi
    ...
    echo -ne " [apply..."
    start=$(date +%s)
    if terraform apply -input=false -no-color tfplan >/dev/null 2>/tmp/tf_error; then
      duration=$(( $(date +%s) - start ))
      printf " ${GREEN}DONE${NC} (%02dm%02ds)]\n" $((duration/60)) $((duration%60))
    else
      duration=$(( $(date +%s) - start ))
      printf " ${RED}FAIL${NC} (%02dm%02ds)]\n" $((duration/60)) $((duration%60))
      echo -e "${RED}❌ APPLY ERROR in $display_path:${NC}"
      cat /tmp/tf_error
      popd >/dev/null
      return 1
    fi
  ...
  1|2)
    ...
    for i in "${!selected_projects[@]}"; do
      ...
      if ! run_terraform "$path" "$([[ "$choice" == "1" ]] && echo "apply" || echo "destroy")"; then
        echo -e "\n${RED}⛔ Operation failed on $path - STOPPING ALL OPERATIONS${NC}"
        exit 1
      fi
    done
